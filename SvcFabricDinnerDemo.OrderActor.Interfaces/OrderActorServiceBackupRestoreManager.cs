...
namespace SvcFabricDinnerDemo.OrderActor.Interfaces
{
    public class OrderActorServiceBackupRestoreManager : IOrderActorServiceBackupRestoreManager
    {
        ...
        public async Task TakeFullBackupAsync(string nameOfBackupset)
        {
            ...
            {
                var partitions = await client.QueryManager.GetPartitionListAsync(OrderActorProxy.ServiceUrl);
                foreach (var partition in partitions)
                {
                    ...
                    var actorServiceProxy = ActorServiceProxy.Create<IOrderActorService>(OrderActorProxy.ServiceUrl, actorpartition.LowKey);
